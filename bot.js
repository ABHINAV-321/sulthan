...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('sulthan is Working😍 ' + config.WORKTYPE + ' 𝚗𝚘𝚠 🍃'));
          if (config.LANG == 'EN' || config.LANG == 'ML') {
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    await conn.sendMessage(conn.user.jid, fs.readFileSync("./photo/sulthan.jpg"), MessageType.image, { caption: `*ʜᴇʏ ʙʀᴏ 𝚂𝚄𝙻𝚃𝙷𝙰𝙽 ᴡᴏʀᴋɪɴɢ ` + config.WORKTYPE + ` 🥂* \n\n*ᴛʜᴀɴᴋʏᴏᴜ ғᴏʀ ᴜsɪɴɢ 𝚜𝚞𝚕𝚝𝚑𝚊𝚗🥂*`});
                } else {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '*◄━⦁💝𝚜𝚞𝚕𝚝𝚑𝚊𝚗💝⦁━►*' + degisiklikler + '```', MessageType.text
                    ); 
                } 
          }
    });//thanx afnanplk
    setInterval(async () => { 
        ...
        var ansk = 'https://gist.githubusercontent.com/ABHINAV-321/b403f9b0707130d9f2e6aab0efa8295c/raw/0c8cdde811104171f3f746300c9f908b993bcc61/gistfile1.txt'
...
*/
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ sulthan ~______~*' +
                                    '\n*sulthan ERROR🔰' +
                                    '\n\n*⚠️ ' + error + '*\n'
                                    , MessageType.text);
                            }
