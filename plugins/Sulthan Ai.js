...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Sulthan') && conf.TALKING_SULTHAN !== 'true') {        
...
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Sulthan സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
...
Asena.addCommand({ pattern: 'sulthan ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.sulthan on / off' }, (async (message, match) => {
    var sulthan_status = `${conf.TALKING_SULTHAN}`
    if (match[1] == 'on') {
        if (sulthan_status == 'true') {
            return await message.client.sendMessage(message.jid, '*' + already_on + '*', MessageType.text)
        }
        else {
            await heroku.patch(baseURI + '/config-vars', { 
                body: { 
                    ['TALKING_SULTHAN']: 'true'
                } 
            });
