...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('maalutty') && conf.TALKING_MAALUTTY !== 'true') {        
        ...
        var finm = message.message.replace('sulthan', '').replace(' ', '')   
        ...
        if (conf.TALKING_SULTHAN == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
            ...
            if (message.jid.includes('-') && (message.mention !== false && message.mention.length !== 0)) {
                ...
                await axios.get('http://api.brainshop.ai/get?bid=161443&key=o4TmUKcCYDFuU5KL&uid=' + unique_ident + '&msg=' + uren).then(async (response) => {
                    ...
                    await message.client.sendMessage(message.jid,'*' + fins +'*', MessageType.text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(message.jid ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "*sulthan AI*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 3000, "width": 3000, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./media/image/logo.jpg')}}}});
                })
            }
        }
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel sulthan özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'sulthan yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'sulthan yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'sulthan, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'sulthan, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional sulthan features. Turn your account into a ai chatbot!'
    already_on = 'sulthan artificial intelligence is already fully functional.'
    already_off = 'sulthan artificial intelligence is currently running semi-functional.'
    succ_on = 'sulthan Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'sulthan Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Maalutty സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'sulthan കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'sulthan AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'sulthan പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി sulthan സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Asena.addCommand({ pattern: 'bot ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.talking on / off' }, (async (message, match) => {
